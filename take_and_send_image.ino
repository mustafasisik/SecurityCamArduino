...
static esp_err_t post_something(){  
  ...
  const char *post_url = "http://134.122.93.176/api/scadddata";

  String image_base = Photo2Base64();
  Serial.println("IMAGEBASE64 READY:");
  String imagex = "code=1234567890&regular=no&image=" + image_base;
  const char *post_data = imagex.c_str();
...
static esp_err_t post_regular_photo(){  

  const char *post_url = "http://134.122.93.176/api/scadddata";

  String image_base = Photo2Base64();
  Serial.println("IMAGEBASE64 READY:");
  String imagex = "code=1234567890&regular=yes&image=" + image_base;
  const char *post_data = imagex.c_str();
  esp_err_t res = ESP_OK;
...
void loop() {
  ...
  if (Firebase.getString(firebaseData, "/security")) {
    if (firebaseData.dataType() == "string") {
      Serial.println(firebaseData.stringData());
      if (firebaseData.stringData() == "off"){
        digitalWrite(PIN_LED, HIGH);
        Serial.println("on side");
   
      }else {
        digitalWrite(PIN_LED, LOW);
        Serial.println("off side");
